  ...
  useEffect(() => {
    const fetchPage = async () => {
      try {
        ...
        console.error("Error fetching page:", err.response?.status, err.response?.data || err.message);
      setPage(null); // show "Page not found"
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="text-white bg-black">
      ...
      <div className="relative w-full h-96">
        ...
        <div className="absolute inset-0 bg-black/40 flex flex-col justify-center items-start px-10 md:px-20">
          ...
          <p className="text-lg max-w-2xl">{page.hero.description}</p>
        </div>
      </div>
    ) : (
      <div className="flex flex-col justify-center px-10 md:px-20 py-20 space-y-6">
        <h1 className="text-5xl md:text-7xl font-semibold">
          {page.hero.heading}
        </h1>
        <p className="text-xl max-w-2xl">{page.hero.description}</p>
      </div>
    )} */}
